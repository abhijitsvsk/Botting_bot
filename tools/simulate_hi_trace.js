...
const textBody = process.env.SIM_TEXT || 'Hi';
...
function checkExpected(nodeName, runData, output) {
  ...
  if (nodeName === 'WhatsApp Webhook') {
    ...
    return first?.messageType === 'text' && first?.text === textBody
      ? `OK: received simulated text ${textBody}`
      : `DIVERGED: webhook did not contain text ${textBody}`;
  }
